                ...
                if (_formKey.currentState!.validate()) {
                  ...
                  try{
                    Openapi(interceptors: [SkipInterceptor()])
                        .getAuthApi()
                        .apiAuthAuthorizePost(authorizeRequest: authRequest)
                        .then((value) async {
                          setState(() {
                            isLoading = false;
                          });
                      if (value.statusCode == 200 && value.data != null && value.data?.isAuthorized != null && value.data?.isAuthorized == true) {
                        final data = value.data; //TODO if data is null??
                        await Authorization.storeTokens(
