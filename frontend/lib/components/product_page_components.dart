...
class ProductPanel extends StatefulWidget {
  const ProductPanel({super.key,  required this.post, required this.userDTO});
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: ProductDescCard(widget.post.body??"Description"),
                    ),
                    if(widget.userDTO.id==Authorization.userId)
                      AcceptButton("Delete Post", Colors.white, Colors.red, () {
                        AuthorizedAPISingleton.instance.getPostApi().postsIdDelete(id: widget.post.id ?? "").then((value) {
                          final id = widget.userDTO.id;
                          context.go("/profile_page/$id");
                        });
                      }),
                    Expanded(child: Container())
                  ],
      ...
      child: Padding(
        ...
        child: Center(
          child: Column(
            ...
            children: [
              CircleAvatar(maxRadius: 70, foregroundImage: NetworkImage((imageUrl ?? "https://images.assetsdelivery.com/compings_v2/yehorlisnyi/yehorlisnyi2104/yehorlisnyi210400016.jpg")),),
              Text(
                name,
                style: TextStyle(
                  fontSize: 24.0,
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  var chatUserId = widget.userDTO.id;
                  context.go('/chat_box/$chatUserId');
                  //TODO Contruct the ChatBox with the seller user id
                },
